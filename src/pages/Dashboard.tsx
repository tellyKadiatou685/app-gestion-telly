...
import {
  ...
  RotateCcw, Check, AlertTriangle, Menu, FileSpreadsheet,
} from "lucide-react";
...
import { RangeTotalsModal } from "@/components/RangeTotalsModal";
...
const FinF2Cell = ({ f1, f2Backend, diffBackend }: FinF2CellProps) => {
  ...
  return (
    <span style={{
      ...
      <span style={{ fontSize: 12, color: "#334155", fontWeight: 600 }}>{fmt(f1)}</span>
      ...
      <span style={{
        ...
        <span style={{ fontSize: 8, color: "#16a34a", fontWeight: 900 }}>✓</span>
        <span style={{ fontSize: 9, color: "#16a34a", fontWeight: 800 }}>F2</span>
        <span style={{ fontSize: 12, fontWeight: 700, color: "#15803d" }}>{fmt(f2Backend!)}</span>
      </span>
      <span style={{ color: "#cbd5e1", fontSize: 11 }}>|</span>
      <span style={{ fontSize: 12, fontWeight: 900, color: diffColor }}>
        {fmtSigned(diff)}
      </span>
    </span>
  );
...
const ActionPopover = ({
  ...
  const cfg = {
    edit: {
      title: "Modifier la valeur", icon: <Pencil className="h-3.5 w-3.5" />,
      accent: "#10b981", light: "rgba(16,185,129,0.08)", textColor: "#065f46",
      btn: "bg-emerald-500 hover:bg-emerald-600 text-white", btnLabel: "Enregistrer",
    },
    delete: {
      title: "Supprimer la ligne", icon: <Trash2 className="h-3.5 w-3.5" />,
      accent: "#ef4444", light: "rgba(239,68,68,0.08)", textColor: "#991b1b",
      btn: "bg-red-500 hover:bg-red-600 text-white", btnLabel: "Supprimer",
    },
    reset: {
      title: "Remettre à zéro", icon: <RotateCcw className="h-3.5 w-3.5" />,
      accent: "#f59e0b", light: "rgba(245,158,11,0.08)", textColor: "#92400e",
      btn: "bg-amber-500 hover:bg-amber-600 text-white", btnLabel: "Réinitialiser",
    },
  }[action.mode];
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (action.mode === "delete") {
        await AccountLineService.deleteAccountLine(
          action.supervisorId, action.lineType, action.accountKey, action.targetDate,
        );
      } else if (action.mode === "reset") {
        await AccountLineService.resetAccountLine(
          action.supervisorId, action.lineType, action.accountKey, 0,
        );
      } else {
        const val = parseFloat(inputValue);
        if (isNaN(val) || val < 0) { setError("Valeur invalide"); setLoading(false); return; }
        await AccountLineService.updateAccountLine(
          action.supervisorId, action.lineType, action.accountKey, val, action.targetDate,
        );
      }
      onSuccess();
    } catch (e: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div
        ...
        style={{ border: `1.5px solid ${cfg.accent}40` }}
      >
        <div
          className="flex items-center justify-between px-5 py-4"
          style={{ background: cfg.light, borderBottom: `1px solid ${cfg.accent}25` }}
        >
          <div className="flex items-center gap-3 min-w-0">
            <span
              className="flex items-center justify-center h-8 w-8 rounded-full text-white flex-shrink-0"
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-medium mt-0.5 truncate" style={{ color: cfg.textColor, opacity: 0.75 }}>
                ...
                {isPastDate && (
                  <span className="ml-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-orange-100 text-orange-600">
                    📅 {action.targetDate}
                  </span>
                )}
              </p>
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-black/5 transition-colors flex-shrink-0 ml-2">
          ...
          {(action.mode === "delete" || action.mode === "reset") && (
            <div className="flex items-start gap-2.5 p-3.5 rounded-xl text-sm" style={{ background: cfg.light, color: cfg.textColor }}>
              ...
              <p className="leading-relaxed">
                {action.mode === "delete"
                  ? isPastDate
                    ? "Cette ligne sera supprimée du snapshot historique. Action irréversible."
                    : "Cette ligne sera archivée. Les transactions passées restent intactes."
...
const SupervisorCardComponent = ({
  ...
  const rows = Array.from(
    { length: Math.max(debutRows.length, finRows.length) },
    (_, i) => ({ debut: debutRows[i] || null, fin: finRows[i] || null }),
  );
  ...
  const ActionButtons = ({
    lineType, rowKey, montant,
  }: { lineType: LineType; rowKey: string; montant: number }) => (
    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all duration-150 flex-shrink-0">
      <button
        title="Modifier"
        onClick={(e) => { e.stopPropagation(); open(lineType, rowKey, montant, "edit"); }}
      ...
      {!targetDate && (
        <button
          ...
          onClick={(e) => { e.stopPropagation(); open(lineType, rowKey, montant, "reset"); }}
      ...
      <button
        title="Supprimer"
        onClick={(e) => { e.stopPropagation(); open(lineType, rowKey, montant, "delete"); }}
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                card.status === "ACTIVE" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
              }`}>
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
            <div className="text-right">
              ...
              <p className={`text-sm sm:text-base font-extrabold tabular-nums ${
                grPositive ? "text-green-600" : "text-red-500"
              }`}>
          ...
          <div className="border-t border-border">
            <div className="grid grid-cols-2">
              ...
              <div className="px-3 sm:px-5 py-2 border-b-2 border-b-blue-400" style={{ background: "rgba(59,130,246,0.06)" }}>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[8px] text-blue-400 hidden sm:inline">(F1 | F2 | diff)</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 bg-muted/20 border-b border-border">
              {(["Début", "Fin"] as const).map((label, idx) => (
                <div
                  key={label}
                  className={`flex justify-between px-3 sm:px-5 py-1.5 text-[9px] sm:text-[10px] font-semibold text-muted-foreground uppercase tracking-wider ${idx === 0 ? "border-r border-border" : ""}`}
                >
                  <span>Type / Partenaire</span>
                  <span>Montant</span>
                </div>
              ))}
            </div>
                    ...
                    {row.debut ? (
                      <>
                        ...
                        <div className="flex items-center gap-0.5 flex-shrink-0">
                          <span className="text-[10px] sm:text-sm font-semibold text-foreground tabular-nums whitespace-nowrap">
                            {fmt(row.debut.montant)}
                          </span>
                  ...
                  <div className="group flex items-center justify-between px-2 sm:px-5 py-2 sm:py-3 hover:bg-blue-50/60 transition-colors min-w-0 overflow-hidden gap-1">
                    {row.fin ? (
                      <>
                        ...
                        {!row.fin.key.startsWith("part-") ? (
                          <div className="flex items-center gap-0.5 flex-shrink-0">
                            <FinF2Cell
                              f1={row.fin.montant}
                              f2Backend={(card.comptes as any).sortieF2?.[row.fin.key] ?? null}
                              diffBackend={(card.comptes as any).diffF2F1?.[row.fin.key] ?? null}
                            />
...
const Dashboard = () => {
  ...
  const fetchDashboard = useCallback(async (
    ...
    try {
      ...
      const normalizedData = {
        ...data,
        supervisorCards: data.supervisorCards?.map((card: any) => ({
          ...card,
          comptes: {
            debut:    card.comptes?.debut    ?? {},
            sortie:   card.comptes?.sortie   ?? {},
            sortieF2: card.comptes?.sortieF2 ?? {},
            diffF2F1: card.comptes?.diffF2F1 ?? {},
          },
        })) ?? [],
      };

      setDashboard(normalizedData as AdminDashboard);
      setRefreshKey((k) => k + 1);
    } catch (err: any) {
  ...
  const initials =
    user?.nomComplet?.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() ?? "??";
  ...
  const handleCardRefresh = useCallback(
    () => fetchDashboard(period, customDate, false),
    [fetchDashboard, period, customDate],
  );
  ...
  // ─── Export Excel (appel backend) ───────────────────────────────────────
  // Le fichier est généré côté serveur à partir de getAdminDashboard() —
  // données complètes et fraîches, pas seulement ce qui est affiché en state.
  const handleExport = useCallback(async () => {
    setExporting(true);
    try {
      await TransactionService.exportAdminDashboardExcel(
        period,
  ...
  const featured = dashboard?.globalTotals?.featured ?? (
    dashboard?.globalTotals?.uvMaster
      ? { type: "UV_MASTER", label: "UV Master", ...dashboard.globalTotals.uvMaster }
      : null
  );

  // Liste des superviseurs à passer au modal de cumul sur plage de dates.
  // On la dérive des cards déjà chargées : pas de nouvel appel réseau.
  const supervisorOptions = useMemo(
    () => cards.map((c) => ({ id: c.id, name: c.nom })),
    [cards],
  );
...
/>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center sm:justify-center">
          ...
          <div className="relative bg-card rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm p-5 border border-border shadow-xl max-h-[70vh] flex flex-col">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowSupervisorMenu(false)} className="p-1 rounded-lg hover:bg-muted">
                <X className="h-4 w-4 text-muted-foreground" />
              </button>
            </div>
            <div className="overflow-y-auto flex-1 space-y-1.5">
              {cards.map((card, idx) => (
                <button
                  key={card.id}
                  onClick={() => { setSupervisorIndex(idx); setShowSupervisorMenu(false); }}
                  className={`w-full text-left p-3 rounded-xl transition-all border-2 ${
                    idx === supervisorIndex ? "bg-green-50 border-green-500" : "bg-muted/40 hover:bg-muted border-transparent"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      <p className={`text-sm font-semibold truncate ${idx === supervisorIndex ? "text-green-700" : "text-foreground"}`}>
                        {card.nom}
                      </p>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        GR : {card.totaux.grTotal >= 0 ? "+" : ""}{fmt(card.totaux.grTotal)}
                      </p>
                    </div>
      ...
      {/* ── Modal cumul sur une plage de dates — 100% additif ── */}
      <RangeTotalsModal
        isOpen={showRangeModal}
        onClose={() => setShowRangeModal(false)}
        supervisors={supervisorOptions}
        initialSupervisorId={currentCard?.id ?? null}
      />
      ...
      <main className="flex-1 min-w-0 p-3 sm:p-6 overflow-auto">
        <div className="flex items-center justify-between mb-4 sm:mb-6 gap-2">
          ...
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            ...
            <button
              onClick={() => fetchDashboard(period, customDate, false)}
              disabled={refreshing}
            ...
            <button
              ...
              disabled={!dashboard || loading || exporting}
              title="Télécharger en Excel"
              className="p-1.5 sm:p-2 rounded-lg bg-card border border-input hover:bg-green-50 hover:border-green-300 transition-colors disabled:opacity-50"
            >
              {exporting
                ? <Loader2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-green-600 animate-spin" />
        ...
        {period === "yesterday" && (
          <div className="bg-orange-50 border-l-4 border-orange-400 p-3 rounded-r-lg mb-4 flex items-center gap-2">
            ...
            <p className="text-orange-700 text-xs sm:text-sm font-medium">Mode historique — F2 affiché depuis le snapshot d'hier</p>
          </div>
        )}
          ...
          <>
            ...
            {featured && featured.sorties > 0 && (
              <div className="bg-card rounded-xl border border-border p-3 sm:p-4 mb-4 sm:mb-5 flex items-center justify-between animate-fade-in gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs sm:text-sm font-semibold text-foreground">{featured.label}</span>
                </div>
                <div className="flex items-center gap-3 sm:gap-6">
                  <div className="text-right">
                    <p className="text-[10px] sm:text-xs text-muted-foreground">Solde</p>
                    <StatBadge value={featured.solde} color="green" />
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] sm:text-xs text-muted-foreground">Sorties</p>
                    <StatBadge value={featured.sorties} color="blue" />
                  </div>
                </div>
              </div>
            )}

            {/* Navigation superviseur - BOUTON TOUJOURS VISIBLE */}
            <div className="flex items-center justify-between gap-1.5 sm:gap-3 mb-3 sm:mb-4">
              {totalCards > 0 && (
                <>
                  <button onClick={goPrev} disabled={supervisorIndex === 0} className="p-1.5 sm:p-2 rounded-lg border border-input bg-card hover:bg-muted disabled:opacity-30 transition-colors flex-shrink-0">
                    <ChevronLeft className="h-4 w-4 text-muted-foreground" />
                  </button>
                  <button
                    onClick={() => setShowSupervisorMenu(true)}
                  ...
                  <button onClick={goNext} disabled={supervisorIndex === totalCards - 1} className="p-1.5 sm:p-2 rounded-lg border border-input bg-card hover:bg-muted disabled:opacity-30 transition-colors flex-shrink-0">
              ...
              <Button onClick={() => setShowTransaction(true)} className="gap-1 sm:gap-2 flex-shrink-0 px-2.5 sm:px-4 text-xs sm:text-sm h-8 sm:h-10">
            ...
            {currentCard ? (
              ...
              <div className="text-center py-16 text-muted-foreground">
                ...
                <p className="text-xs mt-2">Cliquez sur "Ajouter Transaction" pour créer votre première transaction</p>
              </div>
            )}
          </>
        )}
