...
// ══════════════════════════════════════════════════════════════════════════════
// Périodes — alignées sur le Dashboard admin : Aujourd'hui / Hier / Date...
// ══════════════════════════════════════════════════════════════════════════════
const PERIODS: { value: Period; label: string; emoji: string }[] = [
  { value: "today",     label: "Aujourd'hui", emoji: "📅" },
  { value: "yesterday", label: "Hier",        emoji: "🕐" },
  { value: "custom" as Period, label: "Date...", emoji: "🔎" },
];

type Tab = "dashboard" | "transactions";

// ══════════════════════════════════════════════════════════════════════════════
// FinF2Cell — Lecture seule, 100 % backend.
// Affiche F1 seul, ou F1 | F2 (backend) | diff si finSecondaire persisté.
// La saisie F2 se fait dans le modal de transaction (champ finSecondaire).
// ══════════════════════════════════════════════════════════════════════════════

interface FinF2CellProps {
  f1: number;
  f2Backend?: number | null;
  diffBackend?: number | null;
}

const FinF2Cell = ({ f1, f2Backend, diffBackend }: FinF2CellProps) => {
  const hasF2 = f2Backend != null && f2Backend > 0;
  ...
  const diff = diffBackend != null ? diffBackend : f2Backend! - f1;
  const diffColor = diff >= 0 ? "#16a34a" : "#e11d48";
  ...
  return (
    <span style={{
      ...
      <span style={{ fontSize: 13, color: "#334155", fontWeight: 600 }}>{fmt(f1)}</span>
      ...
      <span style={{
        ...
        <span style={{ fontSize: 9, color: "#16a34a", fontWeight: 900 }}>✓</span>
        <span style={{ fontSize: 10, color: "#16a34a", fontWeight: 800 }}>F2</span>
        <span style={{ fontSize: 13, fontWeight: 700, color: "#15803d" }}>{fmt(f2Backend!)}</span>
      </span>
      <span style={{ color: "#cbd5e1", fontSize: 12 }}>|</span>
      <span style={{ fontSize: 13, fontWeight: 900, color: diffColor }}>
        {fmtSigned(diff)}
      </span>
    </span>
  );
...
const btnActionStyle = (color: string, bg: string, border: string): React.CSSProperties => ({
  ...
  fontSize: 14, fontWeight: 700, color, cursor: "pointer",
...
const ActionModal = ({
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (action === "modifier") {
        await AccountLineService.updateAccountLine(
          supervisorId, lineType, accountKey, parseFloat(newValue), targetDate
        );
      ...
      const isAdmin =
        code === "CREATED_BY_ADMIN" || msg.toLowerCase().includes("administrateur");
      ...
      <div style={{
        ...
        {!action && (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <button onClick={() => setAction("modifier")}
              style={btnActionStyle("#15803d", "#f0fdf4", "#22c55e")}>
              ✏️ Modifier la valeur
            </button>
            <button onClick={() => setAction("reinit")}
              style={btnActionStyle("#d97706", "#fffbeb", "#f59e0b")}>
              🔄 Réinitialiser à 0
            </button>
            <button onClick={() => setAction("supprimer")}
              style={btnActionStyle("#e11d48", "#fff1f2", "#fca5a5")}>
        ...
        {(action === "reinit" || action === "supprimer") && (
          <div style={{
            ...
            {action === "reinit"
              ? `Réinitialiser "${displayLabel}" à 0 ?`
              : `Supprimer définitivement "${displayLabel}" ?`
            }
          </div>
        )}
        ...
        {error && (
          <div style={{
            ...
            color: isAdminError ? "#92400e" : "#9f1239", fontSize: 13,
          }}>
            {isAdminError ? "🔒" : "⚠️"} {error}
            {isAdminError && (
              <div style={{ marginTop: 6, fontSize: 12, opacity: 0.8 }}>
                Contactez un administrateur pour effectuer cette modification.
              </div>
            )}
          </div>
        )}
    ...
    >
      <span style={{ display: "flex", alignItems: "center", gap: 7 }}>
        <span style={{
          ...
          color: "#15803d", display: "inline-flex", alignItems: "center",
      ...
      <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <span style={{ fontSize: 14, color, fontWeight: 700 }}>{fmt(montant)}</span>
        {supId && (
          <span className="sup-row-actions-partner">
            <ActionBtn icon="✏️" title="Modifier"   color="#15803d"
              onClick={() => onAction(lineType, accountKey, montant, name)} />
            <ActionBtn icon="🔄" title="Réinit à 0" color="#d97706"
              onClick={() => onAction(lineType, accountKey, montant, name)} />
            <ActionBtn icon="🗑️" title="Supprimer"  color="#e11d48"
              onClick={() => onAction(lineType, accountKey, montant, name)} />
          </span>
        )}
      </span>
...
// ══════════════════════════════════════════════════════════════════════════════
// DatePickerModal — sélection d'une date précise (aligné sur l'admin)
// ══════════════════════════════════════════════════════════════════════════════
...
const DatePickerModal = ({ tempDate, onChange, onApply, onClose }: DatePickerModalProps) => (
  <div style={{
    position: "fixed", inset: 0, zIndex: 60,
    display: "flex", alignItems: "center", justifyContent: "center", padding: 16,
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 800, color: "#1e293b" }}>Date personnalisée</span>
        </div>
      ...
      <input
        ...
        style={{
          ...
          outline: "none", marginBottom: 16, color: "#334155",
        }}
      />
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button
          ...
          style={{
            ...
            cursor: tempDate ? "pointer" : "not-allowed", opacity: tempDate ? 1 : 0.6,
          }}
        >
          Consulter
        </button>
...
const Superviseur = () => {
  ...
  const load = useCallback(async (p: Period, date?: string) => {
    setLoading(true);
    setError(null);
    try {
      // NB : si getSupervisorDashboard ne supporte pas encore un 3e paramètre
      // de date, il faudra l'ajouter côté service pour que "Date..." fonctionne
      // pleinement (même schéma que TransactionService.getAdminDashboard).
      const data = await TransactionService.getSupervisorDashboard(
        undefined,
        p,
        p === "custom" ? date : undefined
      );
      setDashboard(data);
    } catch (e: unknown) {
  ...
  useEffect(() => { load(period, customDate); }, [period, customDate, load]);
  ...
  const handlePeriodChange = (val: Period) => {
    if (val === ("custom" as Period)) { setTempDate(customDate); setShowDatePicker(true); return; }
    setPeriod(val);
    setCustomDate("");
  };
  ...
  const featured = dashboard?.featured ?? (
    dashboard?.uvMaster
      ? { type: "UV_MASTER", label: "UV Master", ...dashboard.uvMaster }
      : null
  );
  ...
  const targetDate =
    period === "yesterday"
      ...
      : period === ("custom" as Period) && customDate
        ? customDate
        : undefined;

  const periodLabel =
    period === ("custom" as Period) && customDate
      ? new Date(customDate).toLocaleDateString("fr-FR", { day: "2-digit", month: "long", year: "numeric" })
      : PERIODS.find((p) => p.value === period)?.label ?? "Aujourd'hui";
  ...
  const openAction = (
    lineType: LineType, accountKey: string, currentValue: number, label: string
  ) => {
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            ...
            <div className="sup-hide-sm">
              ...
              <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600 }}>Superviseur</div>
            </div>
          </div>
          ...
          <div style={{ flex: 1, display: "flex", justifyContent: "center", gap: 4 }}>
            {([
              ...
              <button key={t.key} onClick={() => setTab(t.key)} style={{
                ...
                fontSize: 14, fontWeight: tab === t.key ? 800 : 600,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
            <button onClick={() => setShowModal(true)} style={{
              ...
              whiteSpace: "nowrap", boxShadow: "0 2px 8px rgba(34,197,94,.3)",
            }}>
              + Transaction
            </button>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          {PERIODS.map((p) => (
            <button key={p.value} onClick={() => handlePeriodChange(p.value)} style={{
              ...
              border: `1.5px solid ${period === p.value ? "#22c55e" : "#e2e8f0"}`,
              background: period === p.value ? "#f0fdf4" : "#fff",
              fontSize: 13, fontWeight: 600,
              color: period === p.value ? "#15803d" : "#64748b",
            }}>
              {p.emoji} {p.label}
            </button>
          ))}
          {loading && <span style={{ fontSize: 13, color: "#22c55e", fontWeight: 600 }}>⏳ Chargement…</span>}
        </div>
        ...
        {period === ("custom" as Period) && customDate && (
          <div style={{
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: "#15803d" }}>
              📅 Données historiques — {periodLabel}
            </span>
            <button
              onClick={() => { setPeriod("today"); setCustomDate(""); }}
              style={{ border: "none", background: "transparent", color: "#16a34a", cursor: "pointer", fontSize: 14 }}
            >
              ✕
            </button>
          </div>
        ...
        {error && (
          <div style={{
            ...
            borderRadius: 12, padding: "12px 16px", color: "#9f1239", fontSize: 14, fontWeight: 600,
          }}>
            ⚠️ {error}
          </div>
        )}
  ...
  <div style={{
    ...
    {loading ? (
      ...
      <>
        ...
        <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 3 }}>Début journée</div>
        <div style={{ fontSize: 16, fontWeight: 900, color: "#15803d" }}>{totaux?.formatted.debutTotal ?? "—"}</div>
      </>
    )}
  </div>
  ...
  <div style={{
    ...
    {loading ? (
      ...
      <>
        <div style={{ fontSize: 22, marginBottom: 5 }}>🌆</div>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 3 }}>Fin journée</div>
        <div style={{ fontSize: 16, fontWeight: 900, color: "#d97706" }}>{totaux?.formatted.sortieTotal ?? "—"}</div>
      </>
    )}
  </div>

  {/* Featured (début + fin individuels) */}
  <div style={{
    background: "#f5f3ff", border: "1px solid #8b5cf630",
    borderRadius: 14, padding: "16px 12px", textAlign: "center",
    boxShadow: "0 2px 8px rgba(0,0,0,.04)",
  }}>
    {loading ? (
      ...
      <>
        ...
        <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 6 }}>
          {featured?.label ?? "UV Master"}
        </div>
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 8 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 2 }}>Début</div>
            <div style={{ fontSize: 14, fontWeight: 900, color: "#7c3aed" }}>
              {fmt(featured?.personal?.debut ?? 0)}
            </div>
          </div>
          <div style={{ color: "#cbd5e1", fontSize: 14 }}>→</div>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 2 }}>Fin</div>
            <div style={{ fontSize: 14, fontWeight: 900, color: "#7c3aed" }}>
              {fmt(featured?.personal?.sortie ?? 0)}
            </div>
          </div>
        </div>
      </>
            ...
            {!loading && totaux && (
              <div style={{
                background: totaux.grTotal >= 0 ? "#f0fdf4" : "#fff1f2",
                border: `1px solid ${totaux.grTotal >= 0 ? "#22c55e30" : "#f4304030"}`,
                borderRadius: 14, padding: "14px 18px",
                display: "flex", alignItems: "center", justifyContent: "space-between",
              }}>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: ".07em" }}>
                    Gain / Reste (G/R)
                  </div>
                  <div style={{ fontSize: 25, fontWeight: 900, color: totaux.grTotal >= 0 ? "#15803d" : "#e11d48", marginTop: 3 }}>
              ...
              <div style={{
                ...
                <div className="sup-table-wrap">
                  ...
                  <div style={{ borderRight: "1px solid #f1f5f9" }}>
                    <div style={{ padding: "9px 16px", borderBottom: "2px solid #22c55e", display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#22c55e", display: "inline-block" }} />
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#15803d", textTransform: "uppercase", letterSpacing: ".06em" }}>Début</span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr auto", padding: "5px 16px", fontSize: 11, fontWeight: 800, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".06em", borderBottom: "1px solid #f8fafc", background: "#fafafa" }}>
                      <span>Type / Partenaire</span><span>Montant</span>
                    </div>

                    {accountEntries.length === 0 && !hasPartners && (
                      <div style={{ padding: "18px 16px", color: "#cbd5e1", fontSize: 14, textAlign: "center" }}>—</div>
                    )}
                    ...
                    {accountEntries.map(([key, debut]) => (
                      <div key={key} className="sup-row" style={{ display: "grid", gridTemplateColumns: "1fr auto", padding: "9px 16px", alignItems: "center", borderBottom: "1px solid #f8fafc" }}>
                        <span style={{ display: "flex", alignItems: "center", gap: 7 }}>
                          ...
                          <span style={{ fontSize: 14, color: "#334155", fontWeight: 600 }}>{ACCOUNT_LABELS[key] ?? key}</span>
                        </span>
                        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <span style={{ fontSize: 14, color: "#334155", fontWeight: 700 }}>{fmt(debut as number)}</span>
                          {sup?.id && (
                            <span className="sup-row-actions">
                              <ActionBtn icon="✏️" title="Modifier"   color="#15803d" onClick={() => openAction("debut", key, debut as number, ACCOUNT_LABELS[key] ?? key)} />
                    ...
                    {partnerRows.filter((p) => p.depots > 0).map(({ name, depots }) => (
                      <PartnerRow key={`debut-part-${name}`} name={name} montant={depots} lineType="debut" supId={sup?.id} color="#15803d" onAction={openAction} />
                    ))}
                  </div>
                  ...
                  <div className="sup-col-fin">
                    <div style={{ padding: "9px 16px", borderBottom: "2px solid #3b82f6", display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#1d4ed8", textTransform: "uppercase", letterSpacing: ".06em" }}>Fin</span>
                      <span style={{ fontSize: 10, color: "#93c5fd", fontWeight: 700 }}>(F1 | F2 | diff)</span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr auto", padding: "5px 16px", fontSize: 11, fontWeight: 800, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".06em", borderBottom: "1px solid #f8fafc", background: "#fafafa" }}>
                      <span>Type / Partenaire</span><span>Montant</span>
                    </div>
                    ...
                    {accountEntries.map(([key]) => {
                      const sortie    = (dashboard.comptes.sortie[key]     ?? 0) as number;
                      const f2Backend = (dashboard.comptes.sortieF2?.[key] ?? null) as number | null;
                      const diffBack  = (dashboard.comptes.diffF2F1?.[key] ?? null) as number | null;
                      return (
                        <div key={key} className="sup-row" style={{ display: "grid", gridTemplateColumns: "1fr auto", padding: "9px 16px", alignItems: "center", borderBottom: "1px solid #f8fafc" }}>
                          <span style={{ display: "flex", alignItems: "center", gap: 7 }}>
                            <span style={{ fontSize: 15 }}>{ACCOUNT_ICONS[key] ?? "📦"}</span>
                          ...
                          <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                            {sortie > 0 ? (
                              <FinF2Cell f1={sortie} f2Backend={f2Backend} diffBackend={diffBack} />
                            ) : (
                              <span style={{ fontSize: 14, color: "#cbd5e1", fontWeight: 700 }}>—</span>
                            )}
                            {sup?.id && sortie > 0 && (
                              <span className="sup-row-actions">
                                <ActionBtn icon="✏️" title="Modifier"   color="#15803d" onClick={() => openAction("sortie", key, sortie, ACCOUNT_LABELS[key] ?? key)} />
                    ...
                    {partnerRows.filter((p) => p.retraits > 0).map(({ name, retraits }) => (
                      <PartnerRow key={`fin-part-${name}`} name={name} montant={retraits} lineType="sortie" supId={sup?.id} color="#e11d48" onAction={openAction} />
                    ))}
                  </div>
                </div>
                ...
                {totaux && (
                  <div className="sup-totaux" style={{ borderTop: "2px solid #f1f5f9" }}>
                    <div style={{ padding: "12px 16px", textAlign: "center", background: "#f0fdf4", borderRight: "1px solid #f1f5f9" }}>
                      <div style={{ fontSize: 10, fontWeight: 800, color: "#16a34a", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 3 }}>Début</div>
                      <div style={{ fontSize: 15, fontWeight: 900, color: "#15803d" }}>{fmt(totaux.debutTotal)}</div>
                    </div>
                    <div style={{ padding: "12px 16px", textAlign: "center", background: totaux.grTotal >= 0 ? "#16a34a" : "#dc2626" }}>
                      <div style={{ fontSize: 10, fontWeight: 800, color: "rgba(255,255,255,.8)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 3 }}>GR Total</div>
                      <div style={{ fontSize: 15, fontWeight: 900, color: "#fff" }}>{fmt(totaux.grTotal)}</div>
                    </div>
                    <div style={{ padding: "12px 16px", textAlign: "center", background: "#eff6ff", borderLeft: "1px solid #f1f5f9" }}>
                      <div style={{ fontSize: 10, fontWeight: 800, color: "#2563eb", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 3 }}>Fin</div>
                      <div style={{ fontSize: 15, fontWeight: 900, color: "#1d4ed8" }}>{fmt(totaux.sortieTotal)}</div>
                    </div>
                  </div>
                )}
              </div>
        ...
        {tab === "transactions" && (
          <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f1f5f9", boxShadow: "0 2px 10px rgba(0,0,0,.04)", overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", borderBottom: "1px solid #f8fafc", fontSize: 14, fontWeight: 800, color: "#334155" }}>
            ...
            {loading ? (
              ...
              <div style={{ padding: "48px 20px", textAlign: "center", color: "#94a3b8", fontSize: 15 }}>
              ...
              dashboard.recentTransactions.map((tx) => {
                ...
                return (
                  <div key={tx.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 16px", borderBottom: "1px solid #f8fafc" }}>
                    <div style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, background: isPos ? "#f0fdf4" : "#fff1f2", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>
                      {isPos ? "⬆️" : "⬇️"}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: "#1e293b" }}>{TX_LABEL[tx.type] ?? tx.type}</span>
                        {isPartner && (
                          <span style={{ fontSize: 11, fontWeight: 700, padding: "1px 6px", borderRadius: 6, background: "#f0fdf4", color: "#15803d", border: "1px solid #22c55e20" }}>
                            🤝 Partenaire
                          </span>
                        )}
                      </div>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 900, color: isPos ? "#15803d" : "#e11d48", whiteSpace: "nowrap" }}>
            ...
            {!loading && partnerTransactions.length > 0 && (
              <div style={{ borderTop: "2px solid #f1f5f9" }}>
                <div style={{ padding: "8px 16px", fontSize: 12, fontWeight: 800, color: "#64748b", textTransform: "uppercase", letterSpacing: ".06em", background: "#f8fafc" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", padding: "12px 16px", gap: 10 }}>
                  <div style={{ textAlign: "center", padding: "10px", borderRadius: 10, background: "#f0fdf4" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#16a34a", textTransform: "uppercase", marginBottom: 3 }}>Total dépôts</div>
                    <div style={{ fontSize: 15, fontWeight: 900, color: "#15803d" }}>
                  ...
                  <div style={{ textAlign: "center", padding: "10px", borderRadius: 10, background: "#fff1f2" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#e11d48", textTransform: "uppercase", marginBottom: 3 }}>Total retraits</div>
                    <div style={{ fontSize: 15, fontWeight: 900, color: "#be123c" }}>
      ...
      {showModal && (
        <TransactionModal
          ...
          onSuccess={() => load(period, customDate)}
        />
      )}
      ...
      {showLogout && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, background: "rgba(15,23,42,.5)", backdropFilter: "blur(6px)" }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 26, maxWidth: 300, width: "100%", textAlign: "center", boxShadow: "0 24px 64px rgba(0,0,0,.15)" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 800, color: "#1e293b", marginBottom: 5 }}>Déconnexion</div>
            <div style={{ fontSize: 14, color: "#64748b", marginBottom: 20 }}>Voulez-vous vraiment vous déconnecter ?</div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowLogout(false)} style={{ flex: 1, padding: "10px", borderRadius: 11, border: "1.5px solid #e2e8f0", background: "#fff", fontSize: 14, fontWeight: 700, color: "#64748b", cursor: "pointer" }}>
                Annuler
              </button>
              <button onClick={handleLogout} style={{ flex: 1, padding: "10px", borderRadius: 11, border: "none", background: "#f43f5e", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer" }}>
                Déconnecter
              </button>
            </div>
          </div>
